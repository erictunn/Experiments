...
// Precompute log(2) for the smooth coloring algorithm.
...
void compute_rows(
    ...
    double y_min,
    double y_step,
    double c_x, // Real part of the constant c for the Julia set.
    double c_y, // Imaginary part of the constant c for the Julia set.
    int max_iterations,
    ...
    for (;;) {
        ...
        // TODO: Refactor to compute julia set instead of mandelbrot.
        const int row_end = std::min(row_start + chunk_size, y_res);
        for (int pixel_y = row_start; pixel_y < row_end; ++pixel_y) {
            const double z_y0 = y_min + static_cast<double>(pixel_y) * y_step;
            float* row = output + static_cast<std::size_t>(pixel_y) * static_cast<std::size_t>(x_res);
            double z_x0 = x_min;
            for (int pixel_x = 0; pixel_x < x_res; ++pixel_x, z_x0 += x_step) {
                // Each pixel is the starting point z_0; c stays fixed for the whole image.
                double zx = z_x0;
                double zy = z_y0;
                double zx2 = 0.0;
                double zy2 = 0.0;
...
PyObject* generate_julia(PyObject* /*self*/, PyObject* args) {
    double x_min = 0.0;
    double x_max = 0.0;
    double y_min = 0.0;
    double y_max = 0.0;
...
if (!PyArg_ParseTuple(
    ...
    for (unsigned int i = 0; i < worker_count; ++i) {
        workers.emplace_back(
            ...
            y_step,
            c_x,
            c_y, // x and y refer to real and imaginary for these purposes.
            max_iterations,
            std::ref(next_row),
            chunk_size
        );
...
PyMethodDef module_methods[] = {
    {"generate", generate_julia, METH_VARARGS, "Generate a Julia set image."},
...
PyMODINIT_FUNC PyInit_julia_lib(void) {
    import_array();
